---
title: Visão geral da API do Adobe Experience Platform Edge Network Server
description: Saiba como usar a API do Adobe Experience Platform Edge Network Server.
keywords: API do Adobe Experience Platform Edge Network Server
exl-id: ccd43ece-ab97-4592-ab19-705feff29dff
feature: APIs/SDKs, Recommendations, Administration & Configuration
TQID: https://experienceleague.adobe.com/Ejcn21VsDvAo-JKc0kU4s5q-yVfDNs-1Xu-RN9VBDFc
product_v2:
  - id: e43347a8-f2c5-4aa4-8623-6f13875d7e3a
feature_v2:
  - id: c93393a4-e558-47e1-992e-c91ed4d480ce
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
topic_v2:
  - id: b5ce8718-c3af-4fdb-a1a9-fca32f83a87c
  - id: d3cdead0-685a-4489-9250-4bb709942f66
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: 07d73101a14b986fa9b016350c1ddeac0df4fdc2
workflow-type: tm+mt
source-wordcount: 121
ht-degree: 0%

---

# Visão geral da [!UICONTROL API do Adobe Experience Platform Edge Network Server]

O [!UICONTROL Adobe Experience Platform Edge Network] fornece uma maneira otimizada para os clientes interagirem com qualquer serviço do [!DNL Adobe Experience Cloud] ou do [!UICONTROL Adobe Experience Platform Edge].

A [!UICONTROL API do Edge Network Server] pode ser usada para diversos casos de uso de coleta de dados, personalização, publicidade e marketing. A API do servidor pode ser usada em servidores, dispositivos IoT, decodificadores de sinais e em vários outros dispositivos.

Para obter mais informações, consulte [visão geral da API do Edge Network Server](https://experienceleague.adobe.com/docs/experience-platform/edge-network-server-api/overview.html){target=_blank} na *documentação da API do Edge Server*.
