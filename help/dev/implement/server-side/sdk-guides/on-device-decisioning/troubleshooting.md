...
Para o SDK Node.js, um objeto `logger` deve ser fornecido.
...
Para Java SDK `logRequests` em `ClientConfig` deve ser habilitado.
...
## &#x200B;2. Verificar se [!DNL Target]Rastreamentos está habilitado
...
## Inspecionar erros de notificação de envio
...
* Se você definir uma atividade em um espaço de trabalho/propriedade e não incluir o mesmo ao configurar o SDK, isso poderá fazer com que o `rules.josn` do espaço de trabalho padrão seja baixado, que pode usar a mbox na seção `remoteMboxes`.
