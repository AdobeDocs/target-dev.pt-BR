...
description: Use o [!UICONTROL adobe.target.getOffers()] e suas opções para a função [!DNL Adobe Target] Biblioteca at.js para disparar solicitações para obter vários [!DNL Target] ofertas. (at.js 2.x)
title: Como usar o [!UICONTROL adobe.target.getOffers()] Função?
feature: at.js
...
| --- | --- | --- | --- |
| `consumerId` | String | Não | O valor padrão é a mbox global do cliente se não for fornecida. Essa chave é usada para gerar a ID de dados complementares (SDID) usada para integração A4T.<P>Ao usar `getOffers()`, cada chamada gera uma nova SDID. Se você tiver várias solicitações de mbox na mesma página e quiser preservar a SDID (para que corresponda à SDID da target-global-mbox e da [!DNL Adobe Analytics] SDID), use o `consumerId` parâmetro.<P>Se `getOffers()` O inclui três mboxes (chamadas de &quot;mbox1&quot;, &quot;mbox2&quot; e &quot;mbox3&quot;) e incluem: `consumerId: "mbox1, mbox2, mbox3"` no `getOffers()` chame. |
...
>[!NOTE]
>
>Consulte o [Documentação da API de entrega](/help/dev/implement/delivery-api/overview.md) para obter informações sobre os tipos aceitáveis para todos os campos listados abaixo.
...
| Solicitação > executar > mboxes > mbox > ordem > purchasedProductIds | Não | Nenhum valor em branco.<P>Tamanho máximo de cada valor = 50.<P>Concatenado e separado por vírgula.<P>Tamanho total das IDs de produto `<=` 250. | Recupere as ofertas de determinada mbox com as IDs do produto adquirido da ordem especificadas. |

## Chame [!UICONTROL getOffers()] para todas as exibições
...
## Chame [!UICONTROL getOffers()] para tomar uma decisão no dispositivo
...
## Chame [!UICONTROL getOffers()] para recuperar as exibições mais recentes com os parâmetros e parâmetros do perfil transmitidos
...
## Chame [!UICONTROL getOffers()] para recuperar as mboxes com os parâmetros e parâmetros do perfil transmitidos.
...
## Chame [!UICONTROL getOffers()] para recuperar a carga do analytics no cliente
...
## Buscar e renderizar dados de várias mboxes via [!UICONTROL getOffers()] e [!UICONTROL applyOffers()]
...
O exemplo a seguir mostra como executar um pageLoad usando [!UICONTROL getOffers()] com a at.js 2.*x*  
