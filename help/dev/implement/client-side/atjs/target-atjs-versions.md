...
description: Veja os detalhes sobre as alterações em cada versão da biblioteca at.js de JavaScript do [!DNL Adobe Target].
...
## at.js versão 2.11.9 (fevereiro de 2026)

* Correção de um problema em que a função applyOffers do at.js não detectava e modificava corretamente os componentes da Web do Lightning (LWC). A biblioteca at.js foi atualizada para detectar componentes LWC e aplicar modificações, conforme aplicável. (TGT-54478)
...
* Os detalhes das métricas de clique do [!UICONTROL Analytics for Target] (A4T) são retornados corretamente ao serem usadas solicitações `prefetch`.
...
* **Log de análise do cliente**: forneça aos clientes controle total sobre como enviar dados de análise para o [!DNL Adobe Analytics], seja no cliente ou no servidor.
  ...
  Para obter mais informações, consulte [Logs [!DNL Analytics] do lado do cliente](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/before-implement.html?lang=pt-BR#client-side).
...
* As ofertas de redirecionamento agora têm suporte automático na integração do [!UICONTROL Analytics for Target] (A4T). A solução alternativa do lado do cliente foi removida. (TNT-30247)
...
* Correção de um problema de falha na configuração da atividade de [!UICONTROL Direcionamento de experiência] no VEC ao alternar entre Procurar e Compor. (TNT-27942)
...
* Adição do tratamento de token de resposta. Para obter mais informações, consulte [Tokens de resposta](https://experienceleague.adobe.com/docs/target/using/administer/response-tokens.html?lang=pt-BR).
...
* A implementação assíncrona significa que integrações herdadas, como [!UICONTROL Test&amp;Target para o plug-in SiteCatalyst], podem não funcionar.
