...
description: Saiba como usar o [!DNL Adobe Target] at.js 2.x para implementar o [!DNL Target] para aplicativos de página única (SPA).
title: Posso implementar o [!DNL Target] para aplicativos de página única (SPA)?
...
## [!DNL Adobe Target]Exibições do e Aplicativos de página única

A variável [!DNL Adobe Target] O VEC para SPA aproveita um novo conceito chamado Exibições: um grupo lógico de elementos visuais que juntos constituem uma experiência de SPA. Por isso, uma SPA pode ser considerada como uma transição entre exibições, em vez das URLs, com base nas interações do usuário. Uma Exibição geralmente pode representar um site inteiro ou elementos visuais agrupados dentro de um site.
...
## Implementação [!DNL Adobe Target] Visualizações

Agora que cobrimos o que [!DNL Adobe Target] As exibições são, podemos aproveitar esse conceito no [!DNL Target] para capacitar os profissionais de marketing a executar testes A/B e XT no SPA por meio do VEC. Isso exigirá uma configuração de desenvolvedor única. Vamos analisar as etapas de configuração.

1. Instale o at.js 2.*x*.

   Primeiro, precisamos instalar o at.js 2.*x*. Essa versão da at.js foi desenvolvida com o SPA em mente. As versões anteriores da at.js não são compatíveis [!DNL Adobe Target] Visualizações e o VEC para SPA.

   Baixe o at.js 2.*x* por meio da [!DNL Adobe Target] Interface do usuário localizada em **[!UICONTROL Administração]** > **[!UICONTROL Implementação]**. at.js 2.*x*  também pode ser implantado por meio de tags na [!DNL Adobe Experience Platform].

1. Implemente o at.js 2.*x* função, `[triggerView()](/help/dev/implement/client-side/atjs/atjs-functions/adobe-target-triggerview-atjs-2.md)` em seus sites.

   Depois de definir as Exibições do SPA onde você deseja executar um teste A/B ou XT, implemente o at.js 2.*x* `triggerView()` com as Exibições transmitidas como parâmetro. Isso permite que os profissionais de marketing usem o VEC para projetar e executar os testes A/B e XT para essas Exibições definidas. Se a função `triggerView()` não estiver definida para essas Exibições, o VEC não detectará as Exibições e, portanto, os profissionais de marketing não poderão usar o VEC para projetar e executar testes A/B.

   >[!NOTE]
   >
   >Para obter suporte de visualização no at.js, [viewsEnabled](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md#viewsenbabled) deve ser definido como verdadeiro, caso contrário, todas as funcionalidades de exibição serão desabilitadas.
...
| 8 | Os dados direcionados correspondem a [!DNL Analytics] dados por meio da SDID e é processado na variável [!DNL Analytics] armazenamento de relatórios.<br />Em seguida, os dados do Analytics podem ser exibidos em [!DNL Analytics] e [!DNL Target] via [!DNL Analytics] para [!DNL Target] (A4T). |
...
| 6 | Os dados do Target são correspondidos aos dados do [!DNL Analytics] pela SDID, e processados no armazenamento de relatório do [!DNL Analytics]. [!DNL Analytics] os dados podem ser exibidos em [!DNL Analytics] e [!DNL Target] pelos relatórios do A4T. |
...
## Use o TriggerView para garantir que o A4T funcione corretamente com at.js 2.x e SPAs

Para garantir que o [ for ](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t.html) (A4T) funcione corretamente com a at.js 2.x, certifique-se de enviar a mesma SDID na solicitação do Target e do Analytics.[!DNL Target][!DNL Analytics]
...
Embora esses exemplos usem código JavaScript, tudo isso pode ser simplificado se você estiver usando um gerenciador de tags, como na [Adobe Experience Platform](/help/dev/implement/client-side/atjs/how-to-deployatjs/implement-target-using-adobe-launch.md).

Se as etapas anteriores forem seguidas, você terá uma solução A4T robusta para SPAs.

## Práticas recomendadas de implementação

As APIs da at.js 2.x permitem personalizar o [!DNL Target] implementação de várias maneiras, mas é importante seguir a ordem correta das operações durante esse processo.
...
| 2 | Carregar at.js 2.x | A at.js 2.x carrega todas as APIs necessárias que você usa para implementar [!DNL Target] solicitações e visualizações. |
| 3 | Executar [!DNL Target] solicitação | Se você tiver uma camada de dados, recomendamos carregar dados críticos necessários para enviar para o [!DNL Target] antes de executar o [!DNL Target] solicitação. Isso permite usar `targetPageParams` para incluir os dados que deseja usar no direcionamento.<P>Quando `pageLoadEnabled` e `viewsEnabled` estão definidos como verdadeiro em [targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md), a at.js solicita automaticamente todo o VEC [!DNL Target] ofertas para você na etapa 2.<P>Observe que `getOffers` O também pode ser usado para obter ofertas de VEC após o carregamento da página. Para isso, verifique se o pedido inclui `execute>pageLoad` e `prefetch>views` na chamada de API. |
| 4 | Chama `triggerView()` | Como a variável [!DNL Target] A solicitação iniciada na Etapa 3 pode retornar experiências para a execução do Carregamento de página e Exibições, verifique se `triggerView()` é chamado depois de [!DNL Target] é retornada e conclui a aplicação das ofertas ao cache. Você deve executar essa etapa apenas uma vez por visualização. |
| 5 | Chame o [!DNL Analytics] sinal de exibição de página | Esse beacon envia a SDID associada às Etapas 3 e 4 para o [!DNL Analytics] para compilação de dados. |
| 6 | Chamada adicional `triggerView({"page": false})` | Essa é uma etapa opcional para estruturas SPA que poderiam renderizar novamente determinados componentes na página sem que ocorresse uma alteração de exibição. Nessas ocasiões, é importante chamar essa API para garantir que [!DNL Target] as experiências são reaplicadas depois que a estrutura SPA renderiza novamente os componentes. Você pode executar essa etapa quantas vezes quiser garantir que [!DNL Target] as experiências persistem em suas visualizações do SPA. |

### Ordem de operação para alteração de exibição do SPA (sem recarregamento de página inteira)

| Etapa  | Ação | Detalhes |
| --- | --- | --- |
| 1 | Chama `visitor.resetState()` | Essa API garante que a SDID seja gerada novamente para a nova visualização à medida que é carregada. |
| 2 | Atualize o cache chamando o `getOffers()` API | Essa é uma etapa opcional a ser realizada se essa alteração de exibição tiver o potencial de qualificar o visitante atual para mais [!DNL Target] atividades ou desqualificá-las de atividades. Nesse ponto, também é possível optar por enviar dados adicionais para o [!DNL Target] para habilitar recursos de direcionamento adicionais. |
| 3 | Chama `triggerView()` | Se você tiver executado a Etapa 2, aguarde o [!DNL Target] solicite e aplique as ofertas ao cache antes de executar esta etapa. Você deve executar essa etapa apenas uma vez por visualização. |
| 4 | Chama `triggerView()` | Se você não tiver executado a Etapa 2, poderá executar essa etapa assim que concluir a Etapa 1. Se você executou a Etapa 2 e a Etapa 3, ignore esta etapa. Você deve executar essa etapa apenas uma vez por visualização. |
| 5 | Chame o [!DNL Analytics] sinal de exibição de página | Esse beacon envia a SDID associada às Etapas 2, 3 e 4 para o [!DNL Analytics] para compilação de dados. |
| 6 | Chamada adicional `triggerView({"page": false})` | Essa é uma etapa opcional para estruturas SPA que poderiam renderizar novamente determinados componentes na página sem que ocorresse uma alteração de exibição. Nessas ocasiões, é importante chamar essa API para garantir que [!DNL Target] as experiências são reaplicadas depois que a estrutura SPA renderiza novamente os componentes. Você pode executar essa etapa quantas vezes quiser garantir que [!DNL Target] as experiências persistem em suas visualizações do SPA. |
...
Consulte [Implementar a at.js 2.x do Adobe Target em um aplicativo de página única (SPA)](https://experienceleague.adobe.com/docs/target-learn/tutorials/experiences/use-the-visual-experience-composer-for-single-page-applications.html) para obter mais informações.

### Uso do VEC para SPA no [!DNL Adobe Target]

>[!VIDEO](https://video.tv.adobe.com/v/26249/?quality=12)

Consulte [Uso do Visual Experience Composer para Single Page Application (SPA VEC) no Adobe Target](https://experienceleague.adobe.com/docs/target-learn/tutorials/experiences/use-the-visual-experience-composer-for-single-page-applications.html) para obter mais informações.
